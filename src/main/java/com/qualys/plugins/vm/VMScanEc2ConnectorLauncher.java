...
public class VMScanEc2ConnectorLauncher {	
    ...
    public VMScanEc2ConnectorLauncher(Run<?, ?> run, TaskListener listener,  
    	...
    	this.apiClient = new QualysVMClient(auth, System.out);        
   	...
   	public JsonObject checkInstanceState(String ec2Id, String accountId) throws Exception {
   		...
   		try {
			...
			if (instanceDetails.has("apiError")) {
				throw new APIException("API response: "+instanceDetails.get("apiError").getAsString() + ". Check EC2 details provided to launch the scan.");
			}
...
} // end of VMScanEc2ConnectorLauncher class